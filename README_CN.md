...
- **确定性结构** — 通过双蓝图格式：AISOP (`.aisop.json`) 和 AISIP (`.aisip.json`)
...
| 特性 | 说明 |
|------|------|
| **双蓝图格式** | `.aisop.json`（Mermaid 流程图）和 `.aisip.json`（JSON 流程）— 两种等价格式定义确定性 Agent 行为 |
...
| **治理哈希** | 对程序中所有 `.aisop.json` / `.aisip.json` 文件的 SHA-256 完整性验证 |
| **AIAP 打包** | `.aiap` 归档格式，含清单、文件哈希和 Code Trust Gate |

## 架构

AIAP 通过三域联邦信任模型运行在双蓝图格式之上 — **AISOP**（Mermaid 流程图）和 **AISIP**（JSON 流程）：
...
│                                                     │
│  aisop.dev (种子层)                                  │
│    定义 .aisop.json 和 .aisip.json 格式规范            │
│    中立、静态、基础性                                  │
│                         ↓                           │
│  aiap.dev (权威层)                                   │
│    定义治理规则、质量标准、                             │
...
│                                      │  验证和安全约束
├──────────────────────────────────────┤
│  AISOP/AISIP — 蓝图语言              │  .aisop.json + .aisip.json 格式
├──────────────────────────────────────┤
...
├── main.aisop.json      # 入口点 — AISOP 格式（Mermaid 流程图）
│   或 main.aisip.json   # 入口点 — AISIP 格式（JSON 流程）
...
> **AISOP vs AISIP**：AISOP 使用 Mermaid 流程图定义控制流；AISIP 使用 JSON 结构定义控制流。两者共享相同的 AIAP 治理层（§0/§2/§5/§6），仅在流程定义（§4）上不同。程序可使用任一格式。
