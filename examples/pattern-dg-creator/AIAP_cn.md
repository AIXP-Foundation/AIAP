...
pattern: D+G
flow_format: "AISOP"
summary: "AIAP Creator — 通过 15 阶段 Pipeline 创建、进化、验证、模拟和管理 AIAP 程序的参考实现。14 个模块，209 个节点，661 个场景 (A-Z, AA)。功能: ThreeDimTest (MF1-MF38)、EVOLVE CAP 自适应复杂度、治理完整性同步、质量历史归档、阶段缓存链验证、AISOP/AISIP/DUAL 支持、格式转换、协议对齐 (MCP/A2A)、steps_summary 管道中间输出 (quiet_mode)、IMPACT_SCAN 影响范围检查、ASSERT_CHAIN_CHECK、CALL_CHAIN_VERIFY、EARLY_BASELINE、VERSION_SYNC_MANIFEST 版本同步清单、辅助模块评分模板、sub_mermaid 覆盖分析、first_verification_mode、模块类型差异化、SNAPSHOT_FRESHNESS_CHECK 快照时效验证、structural_impact_matrix 结构化影响矩阵、MODULE_LEVEL_CHECK 子模块级协议检查、CAP_ADVISORY 容量预警、BOOTSTRAP_ADVISORY 自举验证标记、NEW_FEATURE_FAIL_PATHS 新功能失败路径、模块分类跨阶段复用、evolve_cap_metrics 进化容量指标、VERSION_SYNC_JSON 结构化输出。Pattern D+G，Grade S。"
tools:
  - name: file_system
    required: true
    annotations:
...
benchmark:
  threedimscore: 4.899
  grade: "S"
  simulation_coverage: "A(16)+B(13)+C(10)+D(10)+E(13)+F(8)+G(10)+H(14)+J(4)+K(7)+L(2)+M(22)+N(5)+O(6)+P(10)+Q(12)+R(394)+S(14)+T(13)+U(4)+V(10)+W(6)+X(15)+Y(11)+Z(16)+AA(16) = 661 scenarios"
  total_nodes: 209
  pass_rate: "661/661 (100%) — 0 RED, 10 YELLOW_accepted"
---
...
- **main.aisop.json** — 顶级编排器 (33 节点, fractal_exempt, steps_summary sub_mermaid)
